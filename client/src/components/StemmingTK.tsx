...
export default function StemmingTK() {
  const { 
    parties, 
    selectedParties, 
    selectedTKSeats 
  ...
  const hasMajority = selectedTKSeats >= 76;
    ...
    <Card className={`mb-6 ${hasMajority ? 'bg-green-50 dark:bg-green-900 border-green-200 dark:border-green-700' : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-600'} shadow-md`}>
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">
          Tweede Kamer - {selectedTKSeats} zetels geselecteerd
        </CardTitle>
      </CardHeader>
      <CardContent className="p-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="flex flex-1 items-center justify-center">
            ...
            >
              {hasMajority
                ? `Meerderheid (+${selectedTKSeats - 76})`
                : `nog ${76 - selectedTKSeats} zetels nodig`}
            </div>
          </div>
        ...
        <div className="relative w-full bg-gray-200 dark:bg-gray-700 rounded-full h-12 overflow-hidden">
          ...
          <div 
            ...
            style={{ width: `${Math.min((selectedTKSeats / 150) * 100, 100)}%` }}
          />
          ...
          {selectedParties.length > 0 && (
            <div className="absolute inset-0 flex">
              {selectedPartiesData.map((party) => {
                const seats = party.currentSeats;
